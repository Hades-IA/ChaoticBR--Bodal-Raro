...
document.querySelector("#collection").addEventListener('input', function () {
    ...
    document.querySelector('.set-title').innerText = document.querySelector("#collection").value;
...
function download(filename, text) {
    ...
    element.setAttribute('href', 'data:text/plain;charset=utf-8,' + encodeURIComponent(textFormat));
...
function draw(qtn, box = false, collection = false, Notype = false) {
    ...
    let pack1 = packs.map(booster => booster.map(cards => {
        return {
            title: cards.name,
            url: cards.url
        }
    ...
    let makePack = () => {
        ...
        pack1.forEach((cards) => {
            let imgHtml = ''
            cards.forEach(card => {
                imgHtml += `<div> <h2 class="titleCard" >| ${card.title} |</h2> <img class="cardChaotic" src="${card.url}" alt=""></div>`;
            });
            num++
...
function collectionFilter(collectionFilter = false, Notype = false) {
    ...
    const cards = cardsJson.filter(card => {
        ...
        let noRecode = []
        let notype = card.cardtype.filter(cardtype => cardtype.title === `${Notype}`);
        let collection = card.cardtype.filter(cardtype => cardtype.title === `${collectionFilter}`);

        if (collectionFilter && Notype) {
            return collection.length > 0 && noRecode.length === 0 && notype.length === 0;
        }
